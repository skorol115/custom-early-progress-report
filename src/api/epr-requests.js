...
export class EprRequests {
	...
	static async getAllUsers(orgUnitId, gradeItemQueries, isSearchAllCriteria) {
		return await this._post(Routes.AllUsers(orgUnitId, isSearchAllCriteria), JSON.stringify(gradeItemQueries)).then(r => r.json());
	}
	...
	static async getNumUsers(orgUnitId, gradeItemQueries, searchTerm, isSearchAllCriteria) {
		return await this._post(Routes.NumUsers(orgUnitId, searchTerm, isSearchAllCriteria), JSON.stringify(gradeItemQueries)).then(r => r.json());
	}
